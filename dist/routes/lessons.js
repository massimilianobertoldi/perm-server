...
router.put('/:id', requireAuth, requireRole('ADMIN'), async (req, res) => {
    ...
    if (parsed.data.startAt)
        data.startAt = parsed.data.startAt;
    if (parsed.data.endAt)
        data.endAt = parsed.data.endAt;
...
router.post('/enroll', requireAuth, requireRole('STUDENT'), async (req, res) => {
    ...
    if (!permitted)
        return res.status(403).json({ error: 'Not permitted to enroll with this teacher' });
    // Check capacity
    const current = await prisma.enrollment.count({ where: { lessonId } });
    if (current >= lesson.capacity)
        return res.status(409).json({ error: 'Lesson is full' });
    // Create enrollment
...
router.get('/:lessonId/enrollments', requireAuth, requireRole('ADMIN'), async (req, res) => {
    const lessonId = Number(req.params.lessonId);
    const enrollments = await prisma.enrollment.findMany({ where: { lessonId }, include: { student: true } });
    res.json(enrollments);
});
